...
@Component({
  selector: 'app-dialog-search-job-parameter',
  templateUrl: './dialog-search-job-parameter.component.html',
  styleUrls: ['./dialog-search-job-parameter.component.scss'],
})
export class DialogSearchJobParameterComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    private dialogRef: MatDialogRef<DialogSearchJobParameterComponent>,
  ...
  searchAllParameter() {
    this.batchJobService.findSearchJobByCondition('**').then((result) => {
      console.log(result);
  ...
  searchByValue(event) {
    ...
    if (textSearch !== '') {
      this.batchJobService
        .findSearchJobByCondition(this.checkStarAndPercent(textSearch))
        .then((result) => {
          console.log(result);
          if (result.status === 200) {
            const data = result.data;
