...
@Component({
  selector: 'app-dialog-om-search-criteria',
  templateUrl: './dialog-om-search-criteria.component.html',
  styleUrls: ['./dialog-om-search-criteria.component.scss'],
})
export class DialogOmSearchCriteriaComponent implements OnInit {
  ...
  constructor(
    private dialogRef: MatDialogRef<DialogOmSearchCriteriaComponent>,
  ...
  search(e) {
    ...
    this.paymentBlockService
      .searchCriteriaAllByRole(this.data.role, this.data.createBy, value)
      .then((result) => {
        console.log(result);
  ...
  searchStandard() {
    this.paymentBlockService
      .searchCriteriaAllByRole(this.data.role, this.data.createBy, '**')
      .then((result) => {
        if (result.status === 200) {
          this.dataSource = new MatTableDataSource(result.data);
          this.dataSource.sort = this.sort;
    ...
    dialogRef.afterClosed().subscribe((response) => {
      ...
      if (response) {
        if (response.event && response.value === 'Delete') {
          const id = element.id;
          this.paymentBlockService.deleteCriteriaById(id).then((result) => {
            console.log('deleteCriteriaById');
            console.log(result);
            if (result.status === 200) {
