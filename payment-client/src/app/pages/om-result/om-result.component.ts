...
@Component({
  selector: 'app-om-result',
  templateUrl: './om-result.component.html',
  styleUrls: ['./om-result.component.scss'],
})
export class OmResultComponent implements OnInit {
  button = 'Submit';
  ...
  ngOnInit() {
    ...
    this.sidebarService.updateNowPage('om-result');
  ...
  createFormControl() {
    ...
    this.statusToControl = this.formBuilder.control(''); // สถานะใหม่
  ...
  onSearch() {
    ...
    if (this.listSearchValidate.length <= 0) {
      ...
      payload.unblockDateFrom = unblockDate;

      payload.valueOld = form.statusFrom;
      payload.valueNew = form.statusTo;
