...
import { DialogOmFiAreaComponent } from '@shared/component/dialog-om-fi-area/dialog-om-fi-area.component';
...
@Component({
  selector: 'app-om-pto-h',
  templateUrl: './om-pto-h.component.html',
  styleUrls: ['./om-pto-h.component.scss'],
})
export class OmPtoHComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sidebarService.updateNowPage('omPtoH');
  ...
  openDialogOmSaveSearchCriteria(type) {
    ...
    this.omFormCreate.value.listFiArea = this.listFiArea;
  ...
  setDataToOmForm(data) {
    ...
    this.listFiArea = jsonObject.fiArea;
  ...
  defaultOmFormForAreaTreatMent() {
    ...
    this.listCompanyCode = [
      { companyCodeFrom: '01001', companyCodeTo: '89999', optionExclude: false },
      { companyCodeFrom: null, companyCodeTo: null, optionExclude: false },
      { companyCodeFrom: null, companyCodeTo: null, optionExclude: false },
    ];
    this.listDocType = [
    ...
    this.listFiArea = [
      { from: '1000', to: '9999', optionExclude: false },
      { from: null, to: null, optionExclude: false },
      { from: null, to: null, optionExclude: false },
    ];
    this.listDocumentNo = [
    ...
    this.omFormCreate.patchValue({
      ...
      departmentCodeTo: '89999', // รหัสหน่วยงาน
  ...
  openDialogOmVendor() {
    const dialogRef = this.dialog.open(DialogOmVendorComponent, {
      width: window.innerWidth > 1012 ? '50vw' : '80vw',
      data: {
        listVendor: this.listVendor,
        isShowIndex: true, // TO DO FOR CASE PTO-H ONLY NAJA
      },
    });
  ...
  openDialogOmFiArea() {
    ...
    const dialogRef = this.dialog.open(DialogOmFiAreaComponent, {
      data: {
        listFiArea: this.listFiArea,
      },
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result && result.event) {
        if (result.status === 'save') {
          this.listFiArea = result.value;
          console.log(this.listFiArea);

          this.omFormCreate.patchValue({
            provinceCodeFrom: this.listFiArea[0].from,
            provinceCodeTo: this.listFiArea[0].to,
          });
          this.checkOptionExclude();
        }
      }
  ...
  setFiArea(type) {
    ...
    const form = this.omFormCreate.value;
    if (type === 'provinceCodeFrom') {
      if (form.provinceCodeFrom) {
        this.listFiArea[0].from = form.provinceCodeFrom;
      } else {
        if (this.listFiArea.length === 1) {
          this.listFiArea = [{ from: null, to: null, optionExclude: false }];
        } else {
          this.listFiArea.splice(0, 1);
          this.omFormCreate.patchValue({
            provinceCodeFrom: this.listFiArea[0].from,
          });
        }
      ...
      if (form.provinceCodeTo) {
        this.listFiArea[0].to = form.provinceCodeTo;
      } else {
        this.listFiArea[0].to = '';
        this.omFormCreate.patchValue({
          provinceCodeTo: '',
        });
      }
  ...
  isHaveOptionExclude(type) {
    if (type === 'postDate') {
      ...
      return this.listFiArea.some((e) => e.optionExclude);
    }
    return false;
  }
  ...
  getTotalCost() {
    if (!this.dataSource.data || this.dataSource.data.length < 1) return 0;
    return this.dataSource.data.map((item) => item.amount).reduce((prev, next) => prev + next);
  }
