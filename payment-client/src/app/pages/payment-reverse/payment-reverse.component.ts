...
@Component({
  ...
  styleUrls: ['./payment-reverse.component.scss'],
})
export class PaymentReverseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sidebarService.updateNowPage('reverse-payment');
  ...
  onPreSave() {
    ...
    if (this.listValidate.length <= 0) {
      const payload = {
        compCode: form.departmentCodeFrom,
        paymentDocNo: form.documentNo,
        fiscalYear: (form.fiscalYear - 543).toString(),
      };
      this.isSubmitedForm = false;
      this.paymentService.validateMarkComplete(payload).then((result) => {
        if (result.status === 200) {
          const data = result.data;
          if (data && data.fileStatus === 'C') {
            this.snackBar.open('ไม่สามารถกลับรายการเอกสารจ่ายนี้ได้เนื่องจากสถานะตอบกลับเป็นโอนสำเร็จ', '', {
              panelClass: '_warning',
            });
          } else {
